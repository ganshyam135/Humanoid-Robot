...
arduino = serial.Serial("/dev/ttyUSB0", 9600)  # Adjust to your device's port
...
def detect_face():
    ...
    while True:
        ...
        if len(faces) > 0:
            print("😀 Face detected! Waving hi...")
            arduino.write(b'1')  # Signal to Arduino to wave
            speak("Hi there! Please say 'echobot' to start.", "en")
            listen_for_wakeup()
            break
...
def listen_and_respond():
    ...
    while True:  # Continuously listen for queries
        ...
        try:
            ...
            # **Trigger Pointing Gesture if answering direction-related queries**
            if "where" in query or "location" in query or "direction" in query:
                arduino.write(b'3')
